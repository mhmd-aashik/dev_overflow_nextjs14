...
interface Props {
  mongoUserById: string;
}

const Question = ({ mongoUserById }: Props) => {
  ...
  async function onSubmit(values: z.infer<typeof QuestionsSchema>) {
    ...
    try {
      // make an async call to to our API -->> create a question
      // contain all form of data
      await createQuestion({
        title: values.title,
        content: values.explanation,
        tags: values.tags,
        author: JSON.parse(mongoUserById),
        path: pathname,
      });
      ...
      if (tagValue !== "") {
        if (tagValue.length > 15) {
          return form.setError("tags", {
            type: "required",
            message: "Tag name should be less than 15 characters",
          });
        }
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex w-full flex-col">
              <FormLabel className="paragraph-semibold text-dark400_light800 tracking-wide">
